...
`compose/knowledge.yaml` pins — its own container, its own network, no volumes and no
...
- **Everything through Docker Compose.** Change `compose/<domain>.yaml` (services), `compose.yaml` (networks, volumes, the `x-` tier anchors), `config/` or `scripts/` — never the state of a running container. A fresh install must reach the same result.
