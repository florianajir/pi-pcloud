...
Alerts are split across four topics, one per reading mode, so each can be muted, scheduled or exempted from do-not-disturb independently:
...
| `watches` | changedetection | A watched web page changed: a price, a stock status, a published date |

**Subscribe to all four on your phone** — they are independent subscriptions. The Homepage ntfy widget reads a single topic and is pointed at `monitoring`.

`watches` is deliberately not folded into `monitoring`: what arrives there is a price drop at 03:00, not a service that broke, and only a separate subscription can be given its own do-not-disturb rule.

Topics and per-user ACLs are generated by `scripts/ntfy-pre-start.sh` into `config/ntfy/ntfy.env`: each service account gets `rw` on its own topic and nothing else, so a compromised download client cannot publish fake outage or security alerts. Only `ADMIN_USER` can read them all.
...
| **Personal Data** | immich, immich-ml, nextcloud, vaultwarden, kavita, audiobookshelf, freshrss, trilium, changedetection, backrest, backup freshness | 120 s | 3 | each monitor |
| **Media & Downloads** | qbittorrent, stremio, stremio-lan, comet, aiostreams, aiometadata, stremthru, prowlarr, kapowarr, flaresolverr, shelfmark, route qbittorrent | 300 s | 2 (low) | the group only |
