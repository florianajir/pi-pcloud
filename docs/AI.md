...
Then set `PARAKEET_QUANTIZATION=` (empty) on the service in `compose/compose-ai.yaml` so the weights loaded are the weights baked in, and raise `mem_limit` to `3584m`.
