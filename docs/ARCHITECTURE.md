...
| **StremThru** | Its SideKick page only: edits the Stremio addon collection — reorder, enable or disable part of an addon — which Stremio's own UI cannot do, and which is what puts AIOMetadata in front of Cinemeta. Every other StremThru feature is switched off | the owner, in a browser |
