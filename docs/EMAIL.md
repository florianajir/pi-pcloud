...
- Nextcloud's sender domain is always `HOST_NAME`, so the address is `${MAIL_FROM_ADDRESS}@${HOST_NAME}`. Vaultwarden pins `SMTP_SECURITY=starttls` in `compose/cloud.yaml`; change it to `force_tls` there if you move to port 465.
