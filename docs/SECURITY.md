...
| **Open WebUI** | `WEBUI_AUTH_SIGNOUT_REDIRECT_URL` | `compose/compose-ai.yaml` |
| **Grafana** | `GF_AUTH_SIGNOUT_REDIRECT_URL` | `compose/compose-monitoring.yaml` |
...
| **Forgejo** | openid profile email groups | client_secret_basic | one_factor | PKCE (S256) required — goth, the OAuth library underneath, does not implement PKCE, but Forgejo adds the challenge by hand for the `openidConnect` provider specifically (`generateCodeChallenge()` in `routers/web/auth/oauth.go`, appended to the authorize URL by `Source.Callout()`). `groups` **is** read: the auth source is created with `--group-claim-name groups --admin-group admin`, and Forgejo re-evaluates administrator status from that claim on every sign-in, so promoting someone is an LLDAP change. Without the scope nobody is ever an admin, including the first account |
...
| Immich | ✓ | — | ✓ | LAN-only + OIDC only; password login disabled in `config/immich/oauth-config.yaml.template`, so the shared `PASSWORD` is not a way past Authelia into everyone's photos. Re-enable by flipping `passwordLogin.enabled` and restarting — Immich refuses to let the UI override a config file. Second router leaves share paths open — `/share`, `/s/`, `/_app`, three loose root files, the three `/api/server/*` config endpoints, and `/api/assets|download|timeline|albums` *only* when the request carries a `key` or `slug` parameter, never a blanket `/api`. It carries `rate-limit-burst`, because a share page pulls one thumbnail per photo. A **third** router takes the two credential endpoints (`/api/shared-links/me`, `/api/shared-links/login`) — removed from the second router's rule, so the split fails closed if it ever stops loading — onto `immich-share-limit` — 30 per minute, burst 20, bucketed per IPv6 /64 so rotating inside the prefix does not reset it. Immich throttles share passwords nowhere: eight consecutive wrong ones from outside the allowlist answered 401 in ~53 ms each, flat, and there is no setting to enable. Unlike Nextcloud, which recorded the same eight attempts against the real client IP and armed a 25 s delay |
...
| Beszel | ✓ | — | ✓ | LAN-only + OIDC, password login disabled. It had a second, `lan@docker`-free router on `/api/beszel/agent-connect`, the WebSocket an agent holds open to the hub. That traffic is real — `beszel-agent` does reach the hub through Traefik — but it never needed to be public: the agent is `network_mode: host` and pins `beszel.<HOST_NAME>` to `HOST_LAN_IP` with `extra_hosts`, so it arrives from the host's own LAN address, which `lan@docker` admits on the static part of the allowlist rather than on the ISP-delegated `HOST_LAN_SUBNET6`/`WAN_HAIRPIN_IP` entries. Deleted, and the agent's WebSocket is now served by `beszel@docker`. A *remote* agent would need it back, with a limiter |
...
| Forgejo | ✓ | — | ✓ | LAN-only + OIDC. Accounts come from LLDAP only (`ALLOW_ONLY_EXTERNAL_REGISTRATION`), created on first sign-in. No forward-auth: `git clone`/`git push` over HTTPS and `/api/v1` send a personal access token over basic auth and can't pass an interactive portal, as with Kavita's OPDS clients — which is also why `ENABLE_BASIC_AUTHENTICATION` is left on. What that leaves open is anonymous *browsing*, so `REQUIRE_SIGNIN_VIEW=true` closes it: without it every account on the tailnet could read every repository. The healthcheck endpoint (`/api/healthz`) is exempt from that setting, verified against v16.0.5. Git-over-SSH is off — it needs a published host port, and `:22` here is the Pi's own sshd |
...
| Grafana | ✓ | ✓ | ✓ | LAN-only + SSO + OIDC + admin + 2FA. Forward-auth *as well* as its own OIDC, as Homepage and Headplane already do: the dashboards carry per-caller LLM spend and Authelia's own failure counts, and Grafana's `/api` is reachable from anything on `frontend` that dials `:3000`, which no forward-auth in front covers. `GF_AUTH_GENERIC_OAUTH_AUTO_LOGIN=true`, as Homepage does and for the same reason: with the form disabled, `/login` is a page whose only control is a sign-in button, and the forward-auth hop has already established that session. Its signout `rd` therefore ends in `?disableAutoLogin`, or logging out would bounce straight back into a fresh session. Its built-in admin account is inert — `GF_AUTH_BASIC_ENABLED=false` as well as `GF_AUTH_DISABLE_LOGIN_FORM=true`, because Grafana's API accepts HTTP basic auth from that account *regardless* of the login form, which is why no `PASSWORD` is injected into the container. On `frontend` only, unlike the Prometheus it reads: it dials nothing but `prometheus:9090`, so joining `ai` would give the browser-facing half of the pair a path to system-tools and the Docker socket for no reachability it uses |
...
| StremThru | ✓ | partial | — | Two routers, and the split is the inverse of comet's: the **catch-all** is the one carrying forward-auth. It covers SideKick, the dashboard, every configure page, `/v0/store/*` and `/v0/torznab/api` — all browser or operator surfaces, with no headless caller to 401. The second router, `stremthru-addon`, matches `PathPrefix(`/stremio/torz`) && !PathRegexp(`/configure(/|$)`)` and drops `authelia@docker`, because AIOStreams fetches the manifest and stream endpoints server-side and cannot pass an interactive login — and the failure mode is silent: the preset is dropped and the stream list is just shorter. The `!PathRegexp` exclusion is the same one `aiostreams-public` needs and for the same reason — the configure page that matters is the per-userdata one at `/stremio/torz/<userdata>/configure`, which a `PathPrefix(`/stremio/torz/configure`)` carve-out would miss. Verified through Traefik: `/stremio/torz/<ud>/manifest.json` answers without auth, `/stremio/torz/<ud>/configure`, `/stremio/sidekick/`, `/v0/torznab/api` and `/v0/store/user` all redirect to Authelia. **Both routers stay behind `lan@docker`** — nothing off-host needs the addon: AIOStreams reaches it over `frontend`, inside the allowlist, and it is AIOStreams that resolves the results and hands the client its own encrypted `/api/v1/debrid/playback/` link. Publishing it would also put a debrid token on the internet as soon as a store-mode config were used, because torz encodes its userdata into the stream URL it builds. Point torz at the `p2p` store code instead and it returns infoHash-only results with no `url` of its own, so no credential is encoded anywhere — the same shape comet already returns, and the recommended mode here. SideKick asks for a Stremio password and does not keep it: `vault` is the feature that would store a Stremio account server-side, and `HasVault()` checks `IsDisabled`, not `IsEnabled` — so it is off purely because `STREMTHRU_VAULT_SECRET` is empty, and setting that variable would turn it on without the feature line changing. Until then the `authKey` lives in an HttpOnly/Secure cookie in the browser and is read back per request. Turning it on needs a Backrest mount this service deliberately has not got, and `sync` (Stremio↔Trakt) is gated behind it. `STREMTHRU_FEATURE` is an allowlist, so Wrap, Store, List, Newz, the usenet engine, SABnzbd and WebDAV all stay 404 — measured: `/stremio/wrap`, `/stremio/store`, `/stremio/list` and `/stremio/newz` answer 404 while `/stremio/torz/`, `/stremio/sidekick/`, `/v0/torznab/api` and `/v0/meta/id-map/` answer 200. `/v0/store/*` moved from 404 to 400 when `torz` was enabled — it is reachable now, but only behind the forward-auth router, and it still needs store credentials supplied in a request header |
| AIOStreams | partial | — | ✓ | Split in three routers: Comet's two-router shape below, plus a catalogue half on a looser limiter. `/stremio/<uuid>/<encryptedPassword>/*` is public so an addon installed on a Stremio account resolves off-tailnet — that URL *is* the credential. Every configure page is excluded from the public rule by `!PathRegexp(`/configure(/\|$)`)` — a prefix exclusion on `/stremio/configure` alone was not enough, because the page that matters is the per-user one at `/stremio/<uuid>/<encryptedPassword>/configure` — and the only `/api/v1/` path the public rule matches is `/api/v1/debrid/playback/*` — the link AIOStreams hands Stremio at stream-list time and resolves to the debrid CDN with a 307 on click, so without it an off-tailnet client lists streams and plays none of them. The blob in that path is encrypted with `SECRET_KEY`, the same credential-in-URL model as the addon path. The rest of `/api/v1/*` (the SPA, the dashboard, the OIDC callback, the built-in proxy) is not matched at all and stays LAN-only. Only that one URL shape is published, and the other two playback shapes would reproduce the same off-tailnet failure: with the built-in proxy enabled the 307 targets `/api/v1/proxy/*` instead of the CDN, and a usenet-sourced item is handed `/api/v1/usenet/stream/*`. Neither is matched, both deliberately — the proxy would have to move behind gluetun first, and nothing here feeds AIOStreams usenet. The catalogue router carries no `lan@docker` either, so `/static/*` is on the internet too: twelve 15-31 KiB .mp4 status clips (`downloading.mp4`, `403.mp4`, `429.mp4` …) the playback route redirects to instead of failing silently, off local disk and behind `rate-limit-burst` because a player range-requests one on a loop. No forward-auth on either: Stremio fetches manifests programmatically, and the operator surface has Authelia through its own OIDC (deny-by-default group mapping, `AIOSTREAMS_AUTH_REQUIRED=true` gating the configuration page). The public half carries `rate-limit-auth` — each stream request fans out to a dozen upstream addons and a debrid API from the Pi's WAN IP. Local login is **off** (`AIOSTREAMS_OIDC_ALLOW_LOCAL_LOGIN=false`): `POST /api/v1/auth/login` answers 403, so the shared `PASSWORD` is not a second way past Authelia. One `AIOSTREAMS_AUTH` account still exists, but now only as a credential — that flag gates the login route alone, and the built-in proxy and usenet engine authenticate with HTTP Basic, which an SSO identity has not got. It is `ADMIN_USER`/`PASSWORD`, which is sound because nothing accepts it interactively any more; `rotate-password.sh` re-renders it. It is therefore no longer the way back in, and upstream's own advice ("set `AIOSTREAMS_OIDC_ENABLED=false` and restart") does **not** work here — the route checks `allowLocalLogin` with no coupling to `oidc.enabled`. If Authelia is down, flip that one flag back to `true` in `compose/compose-media.yaml` and `up -d` |
| AIOMetadata | partial | — | ✓ | Same split, in three routers. `/stremio/<uuid>/*` is the addon protocol, on `rate-limit-auth`; the artwork routes (`/poster*`, `/logo*`, `/background*`, `/missing_poster.png`) are on `rate-limit-burst` instead — one Stremio home paint is well over a thousand image requests from one browser, which `rate-limit-auth`'s burst of 20 answered with 429 for all but the first few, so most posters simply did not render. Generous rather than absent, because the artwork route embeds an upstream URL and is an image proxy anyone holding an addon URL can aim at the internet; the bytes come off a local disk cache, so it bounds abuse rather than protecting a quota. `POSTER_PROXY_ALLOW_PRIVATE=false` is what stops it being aimed *inward*. `/dashboard` and `/api/*` are at the root and are matched by neither public router; `/configure` is at the root too but is *also* served per user at `/stremio/<uuid>/configure`, so the public rule carries the same `!PathRegexp(`/configure(/\|$)`)` exclusion AIOStreams does. All three stay LAN-only. Its own OIDC gates the operator surface (`AUTH_REQUIRE_SIGNIN=true`), and upstream leaves the addon routes open on purpose: a Stremio client carries no session. `ADMIN_KEY` is the documented bypass when the provider is unreachable, and doubles as the HMAC signing the artwork-proxy URLs. Provider API keys go in `config/aiometadata/api-keys.env` as `BUILT_IN_*` — never the bare names, which `/api/config` serves in full to every visitor even with sign-in required. `rate-limit-auth` on the public half: every catalog page is a fan-out to TMDB/TVDB/MDBList, and MDBList's free tier is 1000 calls a day |
| Comet | ✓ | — | — | One router, entirely behind `lan@docker`. It used to have a public half on `/s/<PUBLIC_API_TOKEN>/` so an addon installed on a Stremio account resolved off-tailnet; nothing installs Comet directly any more. AIOStreams wraps it as a scraper, reaches it over `frontend` and gets back results carrying an `infoHash` and no `url` of their own, so the link a client plays is AIOStreams' `/api/v1/debrid/playback/` — published on that service's router, and encrypted under `SECRET_KEY` where a `/s/<token>/<config>/` URL carries the debrid credentials in plaintext base64. Deleting the router is what takes that key shape off the internet. No forward-auth — Stremio fetches manifests programmatically. `CONFIGURE_PAGE_PASSWORD` stays set even with no public password form: setting it *at all* is what mounts the endpoints under the token prefix, which is baked into the URL AIOStreams is configured with. Its two passwords are generated per-service (`config/comet/comet.env`), never `${PASSWORD}` |

Services with their own account system (Immich, Kavita, Shelfmark, Audiobookshelf, FreshRSS, Trilium, Forgejo) deliberately do **not** stack forward-auth on top of OIDC — their apps and clients cannot complete an interactive portal. Homepage and Headplane are not on that list: neither has such clients — both are only ever opened in a browser, which completes both hops.
...
**Rate limiting** — `rate-limit-auth` (10 req/s average per source IP, burst 20, period 1 s) is on three routers: `lldap`, where it sits *before* the forward-auth middleware and blunts credential stuffing, and `aiostreams-public` and `aiometadata-public`, where it caps the upstream fan-out (see those rows above). It was on `comet-public` too, until that router was deleted. `rate-limit-burst` (100 req/s, burst 200) is the deliberately looser sibling, on `aiometadata-art` and `aiostreams-catalog`: artwork comes off a local disk cache, a catalogue page off a 5-minute cache and AIOStreams' `/static/*` status clips off local disk, none costing anything upstream, while a single Stremio home paint asks for over a thousand images and dozens of catalogues. `rate-limit-auth` stays on the *stream* path, which is the request that fans out to a dozen addons and a debrid API. `immich-public` is on `rate-limit-burst` for the same reason the artwork routes are. A third, service-local middleware exists because neither sibling fits a password field: `immich-share-limit` (30 per minute, burst 20, `ipStrategy.ipv6Subnet=64`) is on `immich-share-auth` alone, and it is the only limiter here sized against guessing rather than against upstream cost — hence the minute-long period and the /64 bucket, without which a per-IP limiter is decorative against a client that can pick a fresh address per request. Note that Traefik is only half the story: AIOStreams rate-limits its own Stremio surfaces too, and both layers were rejecting until `STREMIO_CATALOG_RATE_LIMIT_MAX_REQUESTS` was raised off its default of 30-per-5s.
...
| `AIOSTREAMS_AUTH` | `<ADMIN_USER>:<PASSWORD>` — the one local operator account, kept because the built-in proxy and usenet engine authenticate with HTTP Basic, which an SSO identity has not got. Not an interactive fallback: `AIOSTREAMS_OIDC_ALLOW_LOCAL_LOGIN=false` makes `POST /api/v1/auth/login` answer 403 whatever `AIOSTREAMS_OIDC_ENABLED` says, so the way back in when Authelia is down is flipping that flag to `true` in `compose/compose-media.yaml` and `up -d`. Derived from `PASSWORD`, so `rotate-password.sh` re-renders this file and recreates the container (`rotate_aiostreams`) |
...
`${N8N_RUNNERS_AUTH_TOKEN:-<a hard-coded default>}` in `compose/compose-cloud.yaml` with the variable set nowhere, so every
...
re-renders, restarts Valkey and recreates the four consumers in that order. Authelia, Immich and
Nextcloud are recreated with `--force-recreate`: the secret is a bind-mounted *file* for those three,
so rotating it leaves the service definition byte-identical and a plain `up -d` would find a matching
config hash and do nothing. AIOMetadata is the exception on both counts — it has no `_FILE` variant,
so `scripts/aiometadata-pre-start.sh` is re-run first to put the new password back into `REDIS_URL` in
`config/aiometadata/aiometadata.env`, and the resulting `env_file` change moves the config hash by
itself. Skip either half and that container comes back authenticating with the old password: `NOAUTH`
on every call, and since its sign-in sessions live in Redis, SSO down with nothing in the logs
pointing at the rotation.
...
| AIOMetadata | `REDIS_URL`, with the password inlined, rendered by `scripts/aiometadata-pre-start.sh` into the `0600` gitignored `config/aiometadata/aiometadata.env`. The one consumer with no `_FILE` variant |

Every consumer but AIOMetadata reads it from a file, so `docker inspect` shows none of those holding
it. AIOMetadata is the same environment-only exception as its OIDC client secret above.
